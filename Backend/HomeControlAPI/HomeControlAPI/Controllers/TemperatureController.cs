...
namespace HomeControlAPI.Controllers
{
    ...
    public class TemperatureController : ControllerBase
    {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(NotFound))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(BadRequest))]
        [SwaggerOperation(Summary = "Removes temperature sensor.")]
        ...
        public async Task<IActionResult> UpdateTemperatureSensor([FromRoute] Guid id, [FromBody] TemperatureDTO value)
        {
            if (id == Guid.Empty)
                return BadRequest("Device id can't be empty");
            try
            {
                var temperatureSensor = await _temperatureService.UpdateTemperatureSensor(id, value.Temperature, (Domain.Enums.TemperatureUnit)value.Unit, value.Location);
                return Ok(temperatureSensor);
            }
            catch (InvalidOperationException)
            {
        ...
        public async Task<IActionResult> AddTemperatureSensor([FromBody] TemperatureDTO value)
        {
            try
            {
